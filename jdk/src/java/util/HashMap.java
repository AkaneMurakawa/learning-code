 ...
 *
 * 允许为null，和HashTable差不多，除了不是线程安全的。另外，不保证是有序的。
 * 遍历的顺序既不一定是put()时放入的key的顺序，也不一定是key的排序顺序
 *
 ...
 * important.
 *
 * initialCapacity太高 和 load factor太低对遍历都不好
 *
 ...
 *
 * 负载因子默认是0.75，如果高了可以减少空间，但是会增加遍历开销
 *
 ...
 *
 * 线程不安全的，你可以在外部加锁，或者使用{@link Collections#synchronizedList Collections.synchronizedList}
 *
 ...
 *      a=a-b
 * 4. 判断奇偶： (0 == (a & 1)
 * 5. 当length是2的幂时， hash % length = hash & (length-1)
 * 6. 如何判断一个数是2的次方？答：二进制只有一个是1，其他是0
 *
 *
 * Q: 为什么当length是2的幂时，`hash % length = hash & (length-1)`
 *
 * A: 因为<b>如果一个数是2的次方。二进制只有一个是1，其他是0</b>。当`(length-1)`时，得到的刚好是除了最高位的外的数，
 * 此时，再和一个数进行`&`操作，就相当于取模了。示例：
 * ```
 * 例如：
...
public class HashMap<K,V> extends AbstractMap<K,V>
     ...
     *
     *
     * TREEIFY_THRESHOLD 的设计思想，基于Poisson distribution
     *
     ...
     *
     * 为什么capacity必须是2的幂?
     * 因为当length是2的幂时， hash % length = hash & (length-1)，位运算 & 比 % 的效率高
     */
     ...
     * 负载因子 = 需要的数组大小 / 数组容量
     *
     * 默认值是 0.75，均衡了时间和空间损耗算出来的值
     * 1. 较高的值会减少空间开销（扩容减少，数组大小增长速度变慢），
     * 2. 但增加了查找成本（hash 冲突增加，链表长度变长），
     *
     * 不扩容的条件：数组容量 > 需要的数组大小 /load factor
     *
     * 目的：因为容量 "不足" 时需要扩容，所以最后均衡了时间和空间计算出来的值
     */
     ...
     * shrinkage.
     *
     * 桶上的链表长度 >= 8 时，链表转换为红黑树
     *
     * 为什么 TREEIFY_THRESHOLD 设计是8？
     * 答：因为链表查询的时间复杂度是O(n)，红黑树的查询复杂度是O (log (n))。在链表数据不多时，遍历也很快。
     ...
     *
     * 原文可以查看 本文{@link HashMap}下的 Implementation notes 注释
     *
     * threshold：阈的意思
     *
     */
     ...
     *
     * 数组大小 > 64 时，链表才会转换为红黑树，防止在调整容量和形态时发生冲突
     */
     ...
     *
     * 用于fail-fast，同{@link ArrayList} 和 {@link LinkedList}
     */
     ...
     * @throws IllegalArgumentException if the initial capacity is negative.
     *
     * 基本上，我们用的都是默认的默认的负载因子，因为这个就是设计出来最合适的了，历史沉淀下来的
     * 所以我们主要关心的下面这个构造函数
     */
    ...
    final Node<K,V> getNode(int hash, Object key) {
        ...
        // tab[(n - 1) & hash])表示第一个，这里的第一个并不是指数组的第一个，而是指链表或红黑树的第一个
     ...
     *
     * onlyIfAbsent： 默认false，如果map中已经包含了这个key，则旧的会被替换掉
     * Absent: 缺席的意思
     */
     ...
     * @param onlyIfAbsent if true, don't change existing value 如果为true，则不会改变存在的值
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
                   boolean evict) {
        // tab为数组 (初始化写在if语句中，见下面if ((tab = table) == null || (n = tab.length) == 0)，不友好)
        // n为数组长度
        // i为数组索引下标
        // p为i下标位置的Node
        Node<K,V>[] tab; Node<K,V> p; int n, i;
        // 如果数组为空，使用 resize 方法初始化。
        // 在这里 tab 和 n 被初始化，注意tab = table，n = tab.length
        if ((tab = table) == null || (n = tab.length) == 0)
            n = (tab = resize()).length;
        ...
        // 这种情况则是当前索引位置有值，需要解决hash冲突
            ...
            // 如果是链标，将新节点增加到链表的尾端
                ...
                for (int binCount = 0; ; ++binCount) {
                    // 遍历，p.next == null 表示是尾节点
                    if ((e = p.next) == null) {
                        // 插入到尾部
                        p.next = newNode(hash, key, value, null);
                        // 当链表的长度 s >= 8 时，链表转红黑树。
                        // 其实只有这个条件，链表不一定会转红黑树，数组大小 < 64 则进行扩容， >= 64 则才会转红黑树
                        // TREEIFY_THRESHOLD-1 是因为binCount从0开始的
            ...
            if (e != null) { // existing mapping for key
                V oldValue = e.value;
                // onlyIfAbsent：true，表示存在不替换
                if (!onlyIfAbsent || oldValue == null)
                    e.value = value;
                afterNodeAccess(e); // 空实现
                // 返回插入的值，这里的oldValue并只是旧的值，如果存在且替换，其实也包括了新值value
                return oldValue;
            }
        }
        // 同ArrayList，fail-fast
        ++modCount;
        if (++size > threshold)
            resize();
     ...
     *
     * @return the table
     * 1. 如果旧的容量不为空，如果超过了最大容量，则threshold = Integer.MAX_VALUE；否则扩容为2倍
     * 2. 为0表示第一次初始化，使用默认值
     * 3. 遍历旧的数组，添加到新数组中
     */
    final Node<K,V>[] resize() {
        ...
        if (oldCap > 0) {
            ...
            // oldCap << 1表示扩容为2倍，因为HashMap的容量总是2的次方数
            else if ((newCap = oldCap << 1) < MAXIMUM_CAPACITY &&
                     oldCap >= DEFAULT_INITIAL_CAPACITY)
                // 将threshold扩容为两倍，因为threshold = capacity * load factor，等比
        ...
        if (oldTab != null) {
            ...
            for (int j = 0; j < oldCap; ++j) {
                ...
                if ((e = oldTab[j]) != null) {
                    // 让GC回收
                    oldTab[j] = null;
                    // 如果只包含一个数，没链表和红黑树，则直接赋值
                    if (e.next == null)
                        newTab[e.hash & (newCap - 1)] = e;
                    else if (e instanceof TreeNode) // 如果是红黑树，将树的节点放到newTab中
                        ((TreeNode<K,V>)e).split(this, newTab, j, oldCap);
                    else { // preserve order
                        // 在JDK1.7中，并发情况会造成链表死锁，这里通过四组指针，避免了链表元素倒置
    ...
    final void treeifyBin(Node<K,V>[] tab, int hash) {
        ...
        if (tab == null || (n = tab.length) < MIN_TREEIFY_CAPACITY)
            resize();
        // 链表转换为红黑树
        else if ((e = tab[index = (n - 1) & hash]) != null) { // 最后一个不为null
     ...
     * 1. 节点是红色或黑色
     * 2. 根节点是黑色的
     * 3. 所有叶子的节点是黑色的空节点(null)
     * 4. 每个红色节点的两个子节点都是黑色的(从每个叶子到根的所有路径上不能有两个连续的红色节点)
     * 5. 任意节点到其每个叶子的所有路径都包含相同数量的黑色节点
     *
     * 红黑树：一种平衡二叉树，会自旋平衡
     */
    static final class TreeNode<K,V> extends LinkedHashMap.Entry<K,V> {
        ...
        TreeNode<K,V> prev;    // needed to unlink next upon deletion 前一个节点，删除后需要断开链接
        boolean red; // 是否红节点
